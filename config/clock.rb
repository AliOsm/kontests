...
module Clockwork
  handler do |job|
    ...
    elsif job.eql?('frequent.lc_csa')
      UpdateSitesService.leet_code
      UpdateSitesService.cs_academy
    elsif job.eql?('frequent.ks_he')
      UpdateSitesService.kick_start
  ...
  every(7.minute, 'frequent.lc_csa') # leet_code, cs_academy
  every(11.minute, 'frequent.ks_he') # kick_start, hacker_earth
